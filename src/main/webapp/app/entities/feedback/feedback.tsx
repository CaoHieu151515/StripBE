  ...
  return (
    <div>
      <h2 id="feedback-heading" data-cy="FeedbackHeading">
        <Translate contentKey="sTripBeApp.feedback.home.title">Feedbacks</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="sTripBeApp.feedback.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/feedback/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="sTripBeApp.feedback.home.createLabel">Create new Feedback</Translate>
          </Link>
        </div>
      </h2>
        ...
        {feedbackList && feedbackList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="sTripBeApp.feedback.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('feedbackID')}>
                  <Translate contentKey="sTripBeApp.feedback.feedbackID">Feedback ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('feedbackID')} />
                </th>
                ...
                <th className="hand" onClick={sort('feedbackDescription')}>
                  <Translate contentKey="sTripBeApp.feedback.feedbackDescription">Feedback Description</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('feedbackDescription')} />
                </th>
                ...
                <th className="hand" onClick={sort('feedbackStatus')}>
                  <Translate contentKey="sTripBeApp.feedback.feedbackStatus">Feedback Status</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('feedbackStatus')} />
                </th>
                <th>
                  <Translate contentKey="sTripBeApp.feedback.trip">Trip</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="sTripBeApp.feedback.driver">Driver</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="sTripBeApp.feedback.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {feedbackList.map((feedback, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`sTripBeApp.FeedbackType.${feedback.feedbackType}`} />
                  </td>
                  <td>{feedback.feedbackDescription}</td>
                  <td>{feedback.feedbackRating}</td>
                  <td>
                    <Translate contentKey={`sTripBeApp.FeedbackStatus.${feedback.feedbackStatus}`} />
                  </td>
                  <td>{feedback.trip ? <Link to={`/trip/${feedback.trip.id}`}>{feedback.trip.id}</Link> : ''}</td>
                  <td>{feedback.driver ? <Link to={`/driver/${feedback.driver.id}`}>{feedback.driver.id}</Link> : ''}</td>
                  <td>{feedback.user ? feedback.user.id : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="sTripBeApp.feedback.home.notFound">No Feedbacks found</Translate>
            </div>
          )
        )}
      </div>
