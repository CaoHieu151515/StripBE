  ...
  return (
    <div>
      <h2 id="payment-heading" data-cy="PaymentHeading">
        <Translate contentKey="sTripBeApp.payment.home.title">Payments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="sTripBeApp.payment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/payment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="sTripBeApp.payment.home.createLabel">Create new Payment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {paymentList && paymentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="sTripBeApp.payment.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('paymentID')}>
                  <Translate contentKey="sTripBeApp.payment.paymentID">Payment ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('paymentID')} />
                </th>
                <th className="hand" onClick={sort('amount')}>
                  <Translate contentKey="sTripBeApp.payment.amount">Amount</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('amount')} />
                </th>
                <th className="hand" onClick={sort('paymentDate')}>
                  <Translate contentKey="sTripBeApp.payment.paymentDate">Payment Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('paymentDate')} />
                </th>
                <th className="hand" onClick={sort('paymentStatus')}>
                  <Translate contentKey="sTripBeApp.payment.paymentStatus">Payment Status</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('paymentStatus')} />
                </th>
                <th className="hand" onClick={sort('transactionId')}>
                  <Translate contentKey="sTripBeApp.payment.transactionId">Transaction Id</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('transactionId')} />
                </th>
                <th>
                  <Translate contentKey="sTripBeApp.payment.user">User</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="sTripBeApp.payment.packageDriver">Package Driver</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {paymentList.map((payment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    <Translate contentKey={`sTripBeApp.PaymentStatus.${payment.paymentStatus}`} />
                  </td>
                  <td>{payment.transactionId}</td>
                  <td>{payment.user ? payment.user.id : ''}</td>
                  <td>
                    {payment.packageDriver ? (
                      <Link to={`/package-driver/${payment.packageDriver.id}`}>{payment.packageDriver.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="sTripBeApp.payment.home.notFound">No Payments found</Translate>
            </div>
          )
        )}
      </div>
